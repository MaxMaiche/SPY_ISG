...
# Subplots
def plot_with_subplots(levels, max_scores, min_scores, avg_execution_times, max_execution_times, min_execution_times):
    # Create a figure with two side-by-side subplots
    fig = make_subplots(
        rows=1, cols=2,  # 1 row, 2 columns for side-by-side layout
        subplot_titles=("Max and Min Scores Per Level", "Execution Times Per Level"),
    ...
    fig.add_shape(
        ...
        x0=0.0, x1=0.4, y0=0, y1=1,  # Left graph border
        line=dict(color="black", width=2)  # Border color and width
    )
    fig.add_shape(
        type="rect",
        xref="paper", yref="paper",  # Use paper coordinates for layout-level shapes
        x0=0.6, x1=1.0, y0=0, y1=1,  # Right graph border
        line=dict(color="black", width=2)  # Border color and width
    )
...
def main():
    ...
    for completed in completed_data:
        ...
        if matching_launched:
            ...
            level_scores[level_name].append(completed_score)
    ...
    plot_with_subplots(levels, max_scores, min_scores, avg_execution_times, max_execution_times, min_execution_times)
